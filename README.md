...
Redis Streams 기반 비동기 파이프라인, Spring Batch 통계 집계, JWT Token Versioning 등 실무에서 마주치는 설계 문제들을 직접 고민하고 구현하는 데 초점을 뒀습니다.
...
│       ├── controller/        # AuthController, TodoController, RankingController
│       ├── service/           # AuthService, JwtService, TodoService
│       ├── entity/            # User, Todo, RefreshToken
...
일정 완료/취소 이벤트를 `todo_events`에 원본 그대로 보존하고, 매일 새벽 배치가 `weekly_stats` / `streak_stats`를 집계합니다.
